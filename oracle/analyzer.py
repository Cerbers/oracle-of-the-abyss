...
def count_syllables(word: str) -> int:
    ...
    if word in d:
        #print(f"Syllable count found in dictionary for word: {word}")
        return count_phonetically(word)
    word_lower = word.lower()

    if word_lower in d:
        #print(f"Syllable count found in dictionary for lowercase word: {word_lower}")
    ...
    for char in line:
        if char in vowels:
            if not in_vowel:
                count += 1
                in_vowel = True
                #print(f"Found vowel sequence starting with: {char}")
        else:
            in_vowel = False
    
    #print("this is a simple vowel counter method")
...
def analyze_poem(poem_text: str) -> dict[str, int | list[int]]:
    """Analyzes a poem - syllables per line, total lines"""

    lines = poem_text.strip().split('\n')
    syllables_per_line = [count_syllables_in_line(line) for line in lines]
